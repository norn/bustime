...
def nearest_busstops(bus, x, y, distance_limit=1200, clim=3, DD=None):
    ...
    for r in dd_routes:
        ...
        if not DD['BUSSTOPS_POINTS'].get(r.busstop_id):
            get_busstop_points(bus.city, force=True)
            print("#######################")
...
def rcache_direction_get(bus, uniqueid, DEBUG=False):
    cc_key = "detector_bus_%s" % uniqueid
    llen = REDIS.llen(cc_key)
    if llen >= DIRECTION_BUFFER_SIZE:    
        data = [int(REDIS.lindex(cc_key, i)) for i in range(0, llen) 
            if REDIS.lindex(cc_key, i) != None]
        if DEBUG: print(("Bus %s[%s] directions %s" % (bus.name, uniqueid, data)))
        return most_frequent(data)
    return None

def rcache_direction_set(bus, uniqueid, direction):
    cc_key = "detector_bus_%s" % uniqueid
    REDIS_W.rpush(cc_key, direction)
    while REDIS_W.llen(cc_key) > DIRECTION_BUFFER_SIZE:
        REDIS_W.lpop(cc_key)

def rcache_direction_clear(bus, uniqueid):
    cc_key = "detector_bus_%s" % uniqueid
    while REDIS_W.llen(cc_key) > 0:
        print((REDIS_W.rpop(cc_key)))
...
def ng_detector(bus, x_prev, y_prev, x, y, DEBUG=False, nearest_prev=None, uniqueid:Optional[int] = None, DD=None):
    """
    ...
    common_dir = False if not uniqueid else rcache_direction_get(bus, uniqueid, DEBUG)
    ...
    if el_heroe and uniqueid:
        ...
        rcache_direction_set(bus, uniqueid, el_heroe.direction)
        if not prev_stops:
            rcache_direction_clear(bus, uniqueid)
